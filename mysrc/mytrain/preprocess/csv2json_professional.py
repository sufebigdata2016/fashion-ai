...
def dataframe2json(annotations_csv, category):
    annotations = []
    images = []
...
if __name__ == '__main__':
    ...
    annotations_csv_train["image_id"] = [train_dir + "/" + x for x in annotations_csv_train["image_id"]]
    ...
    annotations_csv_valid["image_id"] = [valid_dir + "/" + x for x in annotations_csv_valid["image_id"]]

    annotations_csv = pd.concat([annotations_csv_train, annotations_csv_valid], axis=0)
    annotations_csv = annotations_csv.sample(frac=1)
    ...
    fdir = f"{file_dir}/mytrain/my{category}_prof"
    ...
    for train_file in annotations_csv_train["image_id"]:
        shutil.copy(train_file, tp_train)
    for valid_file in annotations_csv_valid["image_id"]:
        shutil.copy(valid_file, tp_val)

    json_data_flow_train, need_cols = dataframe2json(annotations_csv_train, category)
    json_data_flow_valid, _ = dataframe2json(annotations_csv_valid, category)
