...
if __name__ == '__main__':
    ...
    args = parser.parse_args("""--model cmu
                                ...
                                --ckptpath /media/yanpan/7D4CF1590195F939/Projects/tf-pose-model/myblouse/tf-pose-2-blouse/cmu_batch:32_lr:0.0001_gpus:1_368x368_/checkpoint
                                --modelpath /media/yanpan/7D4CF1590195F939/Projects/tf-pose-model/myblouse/tf-pose-2-blouse/ 
                                --logpath /media/yanpan/7D4CF1590195F939/Projects/tf-pose-model/myblouse/tf-pose-2-blouse/
                                """.split())
    ...
    with tf.Session(config=config) as sess:
        ...
        while True:
            ...
            if gs_num - last_gs_num >= 100:
                train_loss, train_loss_ll, train_loss_ll_heat, lr_val, summary, queue_size = sess.run(
                    # train_loss_ll_paf
                    [total_loss, total_loss_ll, total_loss_ll_heat, learning_rate, merged_summary_op,
                     # total_loss_ll_paf
                     enqueuer.size()])
                ...
                logger.info(
                    'epoch=%.2f step=%d, %0.4f examples/sec lr=%f, loss=%g, loss_ll=%g, loss_ll_heat=%g, q=%d' % (
                    # loss_ll_paf=%g,
            ...
            if gs_num - last_gs_num2 >= 1000:
                ...
                for images_test, heatmaps, vectmaps in validation_cache:
                    lss, lss_ll, lss_ll_heat, vectmap_sample, heatmap_sample = sess.run(  # lss_ll_paf,
                        [total_loss, total_loss_ll, total_loss_ll_heat, output_vectmap,  # total_loss_ll_paf,
                         output_heatmap],
                ...
                logger.info('validation(%d) %s loss=%f, loss_ll=%f, loss_ll_heat=%f' % (  # loss_ll_paf=%f,
