...
# from network_cmu_simple import CmuNetwork
...
def get_network(type, placeholder_input, sess_for_load=None, trainable=True):
    if type == 'mobilenet':
        ...
        if "cmu" in type:
            ...
            pretrain_path = type
            last_layer = 'Mconv7_stage6_L{aux}'
        elif "mobilenet" in type:
            net = MobilenetNetworkThin({'image': placeholder_input}, conv_width=0.75, conv_width2=0.50,
                                       trainable=trainable)
            pretrain_path = type
            last_layer = 'MConv_Stage6_L{aux}_5'
        else:
    ...
    if sess_for_load is not None:
        if type == 'cmu' or type == 'vgg':
            ...
            ckpt_path = type if "/" in type else os.path.join(_get_base_path(), ckpts[type])
...
def get_graph_path(model_name):
    ...
    graph_path = dyn_graph_path[model_name] if "/" not in model_name else model_name
    for path in (graph_path, os.path.join(os.path.dirname(os.path.abspath(__file__)), graph_path),
                 os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', graph_path)):
