...
class CocoMetadata:
    ...
    def __init__(self, idx, img_url, img_meta, annotations, sigma):
        ...
        # ))
        parts = range(1, TASK_KEY_POINTS)
        transform = list(zip(parts, parts))
...
def get_dataflow(path, is_train, img_path=None, buffer_size=50, if_filp=True):
    ...
    if is_train:
        ...
        # TODO: auto detect buffer_size
        ds = MultiThreadMapData(ds, nr_thread=16, map_func=read_image_url, buffer_size=buffer_size)
...
def get_dataflow_batch(path, is_train, batchsize, img_path=None, buffer_size=50, if_filp=True):
    logger.info('dataflow img_path=%s' % img_path)
    ds = get_dataflow(path, is_train, img_path=img_path, buffer_size=buffer_size, if_filp=if_filp)
